...
try:
    ...
    print(f'[!] Loading bert_score module error')
    pass
...
def cal_length(sentences):
    '''
    One aspect for diversity
    return max/min/avg length of the sentences
    '''
    length = [len(i) for i in sentences]
    max_l = round(max(length), 4)
    min_l = round(min(length), 4)
    avg_l = round(np.mean(length), 4)
...
def cal_BERTScore(refer, candidate):
    # too slow, fuck it
    _, _, bert_scores = score(candidate, refer, lang='zh', rescale_with_baseline=True)
...
def cal_embedding_average(x, y, dic):
    # x and y are the list of the words
    def vecterize(p):
        # do not consider the word embedding of the <unk> token
        vectors = []
        for w in p:
            if w in dic:
                vectors.append(dic[w.lower()])
...
# ========== Multi-view ==========
# def cal_bert_ruber(x, y, model):
#     '''
#     x, y: [batch], a batch of string, model is BERT-RUBER
#     '''
#     s = [f'{x}[SEP]{y}' for i in zip(x, y)]
#     scores = model.score(s)
#     scores = scores.cpu().numpy()
#     return score

def cal_multi_view(x, y, model):
    pass
# ========== Multi-view ==========
