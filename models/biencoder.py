...
class PolyCompEncoder_gate(nn.Module):
    ...
    def predict(self, cid, rid, rid_mask):
        ...
        #     )
        # )
        cross_rep = self.layernorm(rid_rep + cross_rep)    # [S, E]
    ...
    def forward(self, cid, rid, cid_mask, rid_mask):
        ...
        #     )
        # )    # [B, S, 3*E] -> [B, S, E]
        cross_rep = self.layernorm(rid_rep.unsqueeze(0).expand(batch_size, -1, -1) + cross_rep)    # [B, S, E]
...
class PolyCompEncoder_car(nn.Module):
    ...
    def forward(self, cid, rid, cid_mask, rid_mask):
        ...
        # [B, S, E] -> [B, S, 2*E] -> [S, B, 2*E]
        cross_rep = torch.cat(
            [rid_rep.unsqueeze(0).expand(batch_size, -1, -1), rid_rep.unsqueeze(0).expand(batch_size, -1, -1)], dim=-1
        ).permute(1, 0, 2)
        cross_rep = self.dropout(
            torch.tanh(
                self.trs_encoder(
...
class BERTBiEncoderAgent(RetrievalBaseAgent):
    ...
    def __init__(self, multi_gpu, total_step, run_mode='train', local_rank=0, kb=True, model='bertirbi', lang='zh'):
        ...
        if model == 'bertirbi':
            ...
            self.model = PolyCompEncoder_comp(
                self.args['nhead'], 
