...
'''
Keywords enhanced GPT2 dialog model

[CLS] [ Context tokens ] [STP] [ Keywords tokens ] [STP] [ Response tokens ] [STP]

During the reinforcement learning, only the keywords tokens will be used
The [STP] token is the high-level separator, which is similar to the [SEP]
'''

class KWGPT2(nn.Module):

    def __init__(self, vocab_size, unk_id, sep_id, stp_id, topk, topp, 
                 repetition_penalty,
...
class KWGPT2Agent(BaseAgent):
    ...
    def __init__(self, total_steps, multi_gpu, vocab_file='data/vocab/vocab_small', run_mode='train', lang='zh'):
        ...
        self.stp = self.vocab.convert_tokens_to_ids('[STP]')
